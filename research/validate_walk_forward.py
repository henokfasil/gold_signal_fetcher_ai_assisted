...
training window calibrates probabilities; a 48-hour purge separates every
train/calibration/test boundary because labels can look 48 hours forward.
"""
...
def validate(path: Path) -> dict:
    ...
    frame = frame.dropna(subset=["label_profitable", "net_return_pct", *GoldFeatureEngineer.FEATURE_COLS])
    ...
    for year in years:
        ...
        prior = frame[frame.timestamp < test_start - PURGE]
    ...
    return {"status": "REJECT_MODEL" if not all(gates.values()) else "DEVELOPMENT_GATES_PASS",
            "scope": "DEVELOPMENT_ONLY_NOT_UNTOUCHED", "acceptance_gates": gates, "purge_hours": 48,
